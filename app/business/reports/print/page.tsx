...
export default async function ReportsPrintPage({
  ...
  const invoices = periodStart ? allInvoices.filter((i: any) => i.issue_date >= periodStart!) : allInvoices
  const expenses = periodStart ? allExpenses.filter((e: any) => e.date >= periodStart!) : allExpenses
  ...
  return (
    <>
      ...
      {/* A4 Page */}
      <div style={{ background: '#e5e7eb', padding: '32px 0 48px' }}>
        <div className="page" style={{ width: '210mm', margin: '0 auto', background: '#fff', fontSize: '12px', color: '#1f2937', boxShadow: '0 4px 32px rgba(0,0,0,0.15)' }}>
          ...
          <div style={{ background: `linear-gradient(135deg, ${DG}, ${G})`, padding: '28px 36px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <div>
                <div style={{ fontSize: '20px', fontWeight: 800, color: '#fff' }}>{business?.name}</div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                  ...
                  <tbody>
                    {['draft', 'sent', 'paid', 'overdue'].map((s, idx) => {
                      ...
                      const total = filtered.reduce((sum: number, i: any) => sum + Number(i.total_amount || 0), 0)
                      return (
                  ...
                  <tbody>
                    {topCategories.length === 0 ? (
                      ...
                      <tr key={cat} style={{ background: idx % 2 === 0 ? '#fff' : '#f9fafb', borderBottom: '1px solid #f3f4f6' }}>
                        ...
                        <td style={{ padding: '7px 10px', textAlign: 'right', fontWeight: 600, color: '#dc2626' }}>{fmt(total)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                ...
                <tbody>
                  {months.map((m, idx) => {
                    ...
                    const exp  = allExpenses.filter((e: any) => e.date?.slice(0, 7) === m).reduce((s: number, e: any) => s + Number(e.amount || 0), 0)
                    const prof = rev - exp
