...
**27. The Admin UI Toggle Save Bug is a Warning Shot**
During kill-switch verification today, toggling "Expire featured" OFF in the admin UI and clicking Save ended up with `value = 'true'` in the DB — not `'false'`. We haven't root-caused it yet (could be a stale state reference in the save handler, or the toggle not re-rendering before Save fires). It's one data point but the admin panel is how Okoli will run the platform at launch — if any toggle silently saves the wrong value, an operational action (e.g. disabling mentorship during an incident) becomes a production risk. **Action:** Next session, reproduce deterministically and fix. Add an admin audit log entry for every settings save showing key, old value, new value, actor — so we can forensically trace future discrepancies.
