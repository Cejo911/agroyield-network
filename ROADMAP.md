...
> - `/b/[slug]` renders full landing page: cover image, logo, tagline, name, about, products, contact, socials, opening hours, founded year (verified badge flagged as shipped in original F1 note but is not actually rendered — `businesses` has no `is_verified` column; tracked as a Week 2 #2 gap per UNICORN_SPRINT, 18 Apr verification pass)
