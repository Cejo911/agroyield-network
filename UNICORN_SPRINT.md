...
- **18 Apr 2026 (Session 4 hotfix — Recurring page empty-state bug)** — User reported two recurring invoices created but `/business/invoices/recurring` still rendered the empty state. Root cause: the `20260418_recurring_invoices.sql` migration declared `customer_id uuid NOT NULL` with no `REFERENCES customers(id)` clause, so PostgREST couldn't resolve the page's `customers(name)` embed and silently returned an error that the page discarded. Two fixes shipped: (1) new migration `20260418_recurring_invoices_fks.sql` idempotently adds `customer_id → customers(id) ON DELETE RESTRICT` (block orphaning live templates), `user_id → auth.users(id) ON DELETE CASCADE`, and re-asserts `business_id → businesses(id) ON DELETE CASCADE`; ends with `NOTIFY pgrst, 'reload schema'` to refresh the PostgREST cache live. (2) `/business/invoices/recurring/page.tsx` rewritten as a two-query fetch (recurring_invoices first, then customers via `.in()`) rather than relying on the FK embed — robust against future schema-cache hiccups. Also now destructures `error` from the recurring_invoices query and renders a visible red error box instead of silently falling through to "no recurring invoices yet". Scratchpad #48 added (PostgREST nested selects need FKs — and a missing FK silently hides rows). `npx tsc --noEmit` EXIT=0. **⚠️ Migration-paste reminder:** `20260418_recurring_invoices_fks.sql` must be pasted into Supabase SQL editor — this is what unblocks the user's two existing recurring invoices from showing up.
