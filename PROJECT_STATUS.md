...
- **Pre-Beta operational:** ~~paste the 5 Day-0 migrations into Supabase SQL editor before Sunday 26 Apr QA — `20260420_saves.sql`, `20260420_profile_views.sql`, `20260420_profile_experience.sql`, `20260420_open_to_opportunities.sql`, `20260420_global_search_trgm.sql`~~ + `20260421_institution_posting_rls.sql` + `20260421_message_attachments.sql` + `20260421_mentor_approval.sql` — **✅ DONE 21 Apr: Okoli ran all pending migrations in Supabase dashboard in one pass.** Any new migration added from here needs its own individual paste; the `scripts/apply-migrations.sh` GitHub Action from the UNICORN_SPRINT H2 backlog would eliminate this step.
- **~~Task #14 DM composer attachments~~** — ✅ DONE 21 Apr (images + PDF + DOC/DOCX + XLS/XLSX + CSV, 5MB image cap / 10MB file cap, staged preview chip, optimistic-send with rollback, bubble renderers, `message-attachments` public bucket with owner-folder RLS).
- **~~Task #15 Mentor approval gate~~** — ✅ DONE 21 Apr (migration adds `approval_status` / `approved_at` / `approved_by` / `rejection_reason` to `mentor_profiles`; become-mentor inserts pending + surfaces status banner; mentorship browser + `/mentorship/[id]` + directory gate on approved; admin MentorshipTab gains Pending/Approved/Rejected filters with one-click approve and prompted-reason reject; email + Slack notification on both paths; existing active mentors grandfathered as approved via the migration backfill).
- **#14 (calendar)** Sun 26 Apr — QA day, no code changes (pending)
- **#15 (calendar)** Mon 27 Apr — Beta launch morning (pending)
