...
- Added: Scratchpad #49 in ROADMAP.md (Feature-Flag-Default-False + Fire-And-Forget UI = Phantom Success). Two go-forward rules: any "non-blocking" fetch after a successful primary write must still surface non-2xx — "best-effort with visible failure", not "fire and forget"; default-false feature flags need an admin-UI toggle that doesn't require SQL editor access (currently every flip is a manual SQL update with no audit trail). Build the admin flag-toggle UI before the next default-false flag ships (probably #5 Expense OCR).
...
- Next: Unicorn #5 Expense OCR per Week 3 Thu–Sun in UNICORN_SPRINT. Recommend building the admin feature-flag toggle UI first so #5 can launch with a flippable switch from day one.
